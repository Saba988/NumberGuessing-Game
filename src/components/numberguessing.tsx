"use client"
import { useState, useEffect, ChangeEvent, JSXElementConstructor } from "react";
...
export default function NumberGuessing():JSX.Element{
    ...
    const [useGuess,setUerGuess]=useState <string | number>("");
...
const handleGuess=():void=>{
   if ( typeof useGuess ==="number" && useGuess === targetNumber)
{
    setGameOver(true)
} else{
...
const handleUserGuessChange=(e:ChangeEvent<HTMLInputElement>):void=>{
    setUerGuess(parseInt(e.target.value));
};
return(
    <div className="flex flex-cols items-center justify-center h-screen bg-gradient-to-br from-gray-800 to-black ">
